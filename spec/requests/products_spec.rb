...
RSpec.describe 'Products API', type: :request do
  ...
  describe 'POST /products' do
    # valid payload
    let(:valid_attributes) { { title: 'Learn Elm', category_id: category_id } }

    context 'when the request is valid' do
      before { post '/products', params: valid_attributes }

      it 'creates a product' do
        expect({title: json['title'], category_id: json['category']['id']}).to eq({:title => 'Learn Elm', :category_id => category_id })
      end
    ...
    context 'when the request is invalid' do
      before { post '/products', params: { title: '', category_id: nil } }
      ...
      it 'returns a validation failure message' do
        expect(response.body)
            .to match(/Validation failed: Category must exist, Title can't be blank/)
      end
    end
  end
